...
import type { Course, EtatInteressementUtilisateur } from "../../../constantes/types/calendrier";
import type { Role } from "../../../constantes/types/auth";
import { useNotifications } from "../../../contexts/NotificationsContext";
import EtatInteressementAdherent from "../../calendrier/EtatInteressementAdherent";
...
const CHAMPS_INITIAUX: Champs = {
    ...
    etatInteressementUtilisateur: null
};
...
export default function ModalNouvelleCourse({ ancienneDonnees, ouvert, onFermer, setCourses, role }: Props) {
    ...
    function mettreAJour(cle: keyof typeof CHAMPS_INITIAUX, valeur: string | boolean | EtatInteressementUtilisateur) {
    ...
    return (
        <Modal ouvert={ouvert} titre={role == "adherent" ? "Proposer une course" : ancienneDonnees ? "Modifier la course" : "Nouvelle course"} onFermer={onFermer}>
            <form onSubmit={envoyer} className="flex flex-col gap-4">
                ...
                {role == "adherent" &&
                    <Champ label="Votre participation">
                        <EtatInteressementAdherent
                            estInteresse={champs.etatInteressementUtilisateur == "interesse"}
                            participe={champs.etatInteressementUtilisateur == "participe"}
                            setEtat={(valeur: EtatInteressementUtilisateur) => mettreAJour("etatInteressementUtilisateur", valeur)}
